...
function et_ClientCommand(cno,cmd)
	...
	if cmd == "team" then --exploit fix team command
...
--	printf("check_userinfo: [%s]\n",userinfo)
	...
	local n = 0
	for _ in string.gmatch(userinfo,"\\") do
		n = n + 1
	end
	...
	for m in string.gmatch(userinfo,"\\([^\\]*)\\") do
...
--	printf("clientuserinfochanged %d\n",cno)
	...
	for client = 0, (maxclients - 1) do
		...
		if player_name == player_name_cno and player_name_cno ~= "ETPlayer" and player_name_cno ~= "UnnamedPlayer" and cno ~= client and et.gentity_get(cno,"pers.connected") ~= 2 and et.gentity_get(cno,"ps.ping") > 0 then
			et.G_LogPrint(string.format("userinfocheck: client %d badinfo %s\n",cno,"duplicate name"))
			et.trap_DropClient( cno, "duplicate name", 0 )
		elseif player_guid == player_guid_cno and player_guid ~= "NO_GUID" and player_guid ~= "UNKNOWN" and cno ~= client and et.gentity_get(cno,"pers.connected") ~= 2 and et.gentity_get(cno,"ps.ping") > 0 then
			et.G_LogPrint(string.format("userinfocheck: client %d badinfo %s\n",cno,"duplicate guid"))
...
et.G_Printf = function(...)
		et.G_Print(string.format(struct.unpack(arg)))
end
