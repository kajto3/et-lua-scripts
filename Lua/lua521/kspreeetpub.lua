...
-- changed os.date (fixes windows crashs)
-- removed table.getn and changed it to # because it's bugged since lua 5.1.*
-- warmup fix! 
-- added checkteam
...
kmulti_pos      	= "cpm"			-- multi + megakill - position
kmonster_pos    	= "cpm"			-- ultra + monster + ludicrous + holy shit - position
kmultitk_pos		= "cpm"			-- multi TK - position
kspree_pos    		= "cpm"			-- killing spree position
...
record_cmd 			= "!sprees"	-- command to print players with most multi,mega,ultra... kills
...
function et_InitGame(levelTime, randomSeed, restart)
    ...
    et.G_Print(string.format("kspree.lua: loaded %d alltime stats from %s\n", i, kspree_cfg))

    if srv_record then
        i = readRecords(record_cfg)
        et.G_Print(string.format("kspree.lua: loaded %d alltime records from %s\n", i, record_cfg))
    end

    --et.trap_SendConsoleCommand(et.EXEC_NOW,"sets KSpree_version "..version)
    et.G_Print(string.format("bennz's kspree.lua version "..version.." activated...\n"))
	et.G_Print(string.format("kspree.lua: startup: %d ms\n", et.trap_Milliseconds() - func_start))
end
...
function saveStats(file, list)
	...
	local func_start = et.trap_Milliseconds()
    if len == -1 then
        et.G_Print(string.format("kspree.lua: failed to open %s", file))
		et.G_Print(string.format("kspree.lua: readStats(): %d ms\n", et.trap_Milliseconds() - func_start))
        return(0)
    end
	...
	et.G_Print(string.format("kspree.lua: saveStats(): %d ms\n",
                    et.trap_Milliseconds() - func_start))
end
...
function readStats(file)
    ...
    local count      = 0
	if len == -1 then
		et.G_Print(string.format("kspree.lua: no killingspree.txt\n"))
		--saveStats(file, alltime_stats)
		et.G_Print(string.format("kspree.lua: readStats(): %d ms\n", et.trap_Milliseconds() - func_start))
		return(0)
	end
...
function readRecords(file)
	...
	local count      = 0
	if len == -1 then
		et.G_Print(string.format("kspree.lua: no Spree Records \n"))
		et.G_Print(string.format("kspree.lua: readRecords(): %d ms\n", et.trap_Milliseconds() - func_start))
		return(0)
	end
	local filestr = et.trap_FS_Read( fd, len )
	et.trap_FS_FCloseFile( fd )
	...
	et.G_Print(string.format("spree.lua: readRecords(): %d ms\n", et.trap_Milliseconds() - func_start))
	return(count)
end
...
function et_Obituary(victim, killer, mod)
	if gamestate == 0 then
	if mod ~= 0 and mod ~= 5 and mod ~= 21 and mod ~= 24 and mod ~= 25 and mod ~= 31 and mod ~= 32 and mod ~= 33 and mod ~= 34 and mod ~= 36 and mod ~= 38 and mod ~= 39 and mod ~= 52 and mod ~= 53 and mod ~= 54 and mod ~= 39 and mod ~= 22 and mod ~= 23 and mod ~= 35 and killer ~= 1022 and killer ~= 1023 then
    
    local v_teamid = checkteam(victim)
    local k_teamid = checkteam(killer)
    ...
    elseif (v_teamid == k_teamid) then -- team kill
        ...
        if killer ~= 1022 and killer ~= 1023 then -- no world / unknown kills
            ...
            if srv_record and guid ~= "" and not isBot(killer) then
...
function et_RunFrame(levelTime)
	...
	-- wait_table[id] = {lvltime, 2}
	for id, arr in ipairs(wait_table) do
		 local guid = getGuid(id)
		 local m_name = playerName(id)
         ...
         if whichkill == 1 and (startpause + 3100) < ltm then
         	...
         	if srv_record and guid ~= "" and (string.sub (guid, 25,32 )) ~= "00000000" then
...
function et_ClientCommand(id, command)
    ...
    if et.trap_Argv(0) == "say" then
	    ...
	    elseif et.trap_Argv(1) == statsme_cmd and srv_record then
	    	...
	    	et.trap_SendServerCommand( id, "cpm \"^3statsme: ^7"..statsme_msg.." ^7\"\n")
	    	return(1)
		end -- end elseif...
	...
	if et.trap_Argv(0) == msg_command then
        if et.trap_Argv(1) == "" then
            ...
            et.trap_SendServerCommand(id,
                    string.format("cpm \"^#(ksprees):^7 Messages are %s\"",
                            status))
        elseif tonumber(et.trap_Argv(1)) == 0 then
            setKSpreeMsg(id, false)
            et.trap_SendServerCommand(id,
                    "cpm \"^#(ksprees):^7 Messages are now ^8off^7\"")
        else
            setKSpreeMsg(id, true)
            et.trap_SendServerCommand(id,
                    "cpm \"^#(ksprees):^7 Messages are now ^8on^7\"")
        end
        return(1)
    end
		...
		if (#vsaystring) < 2 and last_tk[id] ~= nil and string.lower(vsaystring[1]) == "sorry" then
			if (et.trap_Milliseconds() - tonumber(last_tk[id][2])) < sorry_time then
				...
				for t=0, sv_maxclients-1, 1 do
        			if checkteam(t) == checkteam(id) then
...
function add_qwnage(id, woot)
	...
	if len == -1 then
                    et.G_Print(string.format("kspree.lua: failed to save awards for %s\n", playerName(id)))
    else
		if woot == 1 then
    ...
    if cmd == "ksprees" then
        ...
        for i=0, sv_maxclients-1 do
            ...
            if killing_sprees[i] ~= nil and killing_sprees[i] ~= 0 then
                et.G_Print(string.format("^7ksprees: %d %s^7 (%s)^7\n",
                            killing_sprees[i],
                            playerName(i),
                            teamName(tonumber(checkteam(i)))))
            end
        end
        et.G_Print("^7ksprees: --------------------\n")
        if kmax_id ~= nil then
            et.G_Print(string.format("^7Max: %s^7 with %d\n", playerName(kmax_id), kmax_spree))
        end
        return(1)
    elseif cmd == "kspreesall" then
        et.G_Print("^7Alltime killing sprees:\n")
        for map, arr in pairs(alltime_stats) do
			et.G_Print(string.format("kspreesall: %s: %s^7 with %d kills @%s\n", map, arr[3], arr[1], os.date(date_fmt, arr[2])))
        end
        et.G_Print("^7Alltime killing sprees END\n")
        return(1)
    elseif cmd == "kspreerecords" then
        et.G_Print(string.format("^1kspree_records: %s\n", recordMessage()))
        return(1)
    end
    return(0)
end
