...
function et_RunFrame( levelTime ) 
  if math.mod(levelTime, samplerate) ~= 0 then return end
	if string.find(mapname, "^"..baserace.."") then
		...
		if math.mod(levelTime, 600000) ~= 0 then return end
...
function et_Print(text)
	...
	--AXIS SCORE
	local sbaxc
	for sbaxc in string.gfind(text, "The Axis are ahead by (%x)") do
		et.trap_Cvar_Set( "b_scoreboard_axis", sbaxc )
	end
	
	for sbaxc in string.gfind(text, "The Axis have a commanding lead!") do
		et.trap_Cvar_Set( "b_scoreboard_axis", "5" )
	end
	
	--ALLIES SCORE
	local sbalc
	for sbalc in string.gfind(text, "The Allies are ahead by (%x)") do
		et.trap_Cvar_Set( "b_scoreboard_allies", sbalc )
	end
	
	for sbalc in string.gfind(text, "The Allies have a commanding lead!") do
		et.trap_Cvar_Set( "b_scoreboard_allies", "5" )
	end
	
	--TIED
	local tied
	for tied in string.gfind(text, "The Score is Tied!") do
		et.trap_Cvar_Set( "b_scoreboard_axis", "0" )
		et.trap_Cvar_Set( "b_scoreboard_allies", "0" )
	end
