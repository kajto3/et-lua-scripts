...
--	printf("clientuserinfochanged %d\n",cno)
	...
	for client = 0, (maxclients - 1) do
		...
		if player_name == player_name_cno and player_name_cno ~= "ETPlayer" and player_name_cno ~= "UnnamedPlayer" and cno ~= client and not et.gentity_get(playerID,"pers.connected") == 2 and not et.gentity_get(playerID,"ps.ping") == 0 then
			et.G_LogPrint(string.format("userinfocheck: client %d badinfo %s\n",cno,"duplicate name"))
			et.trap_DropClient( cno, "duplicate name", 0 )
		elseif player_guid == player_guid_cno and player_guid ~= "NO_GUID" and player_guid ~= "UNKNOWN" and cno ~= client and not et.gentity_get(playerID,"pers.connected") == 2 and not et.gentity_get(playerID,"ps.ping") == 0 then
			et.G_LogPrint(string.format("userinfocheck: client %d badinfo %s\n",cno,"duplicate guid"))
