...
--	printf("infocheck %d %d\n", infocheck_client, leveltime)
		...
		if et.gentity_get(infocheck_client,"ps.ping") ~= 0 then
			if ( et.gentity_get( infocheck_client, "inuse" ) ) then
				...
				if ( reason ) then
					...
					local guid = string.upper(et.Info_ValueForKey(et.trap_GetUserinfo(infocheck_client), "cl_guid"))
					global_ban_table[guid] = true
				end
			end
		end
	end
	...
	for cno=0, (maxclients-1) do
		counter[cno] = counter[cno] + 1
		if counter[cno] == 60000 then --lift ban after 1 minute (1 minute = 60000 ms)
			local guid = string.upper(et.Info_ValueForKey(et.trap_GetUserinfo(cno), "cl_guid"))
			global_ban_table[guid] = false
		end
	end
...
-- note IP validity should be enforced by userinfocheck stuff
	...
	local guid = string.upper(et.Info_ValueForKey(et.trap_GetUserinfo(cno), "cl_guid"))
 	if global_ban_table[guid] then 
		return "You are disallowed to reconnect for one minute"
	end
end
-- NoAutoDeclare()
