...
function et_InitGame(levelTime, randomSeed, restart)
	...
	et.G_Printf(""..luaname..".lua: startup: %d ms\n", et.trap_Milliseconds() - func_start)
...
function et_ClientDisconnect( client )
	if isBot1(client) ~= true then
		...
		if counter == numlastseen then 
			table.remove(playertable , tempname)
			counter = counter - 1
		end
...
function seen(client,clientID)
	...
	if len <= 0 then
		...
		for First, Guid, Name in string.gfind(filestr, "(%x+)%s%-%s(%x+)%s%-%s*([^%\n]*)") do
...
function lastseen(client,file)
	...
	for i=1, table.getn(playertable), 1 do
		v = playertable[i]
...
function readdatabase(file)
    ...
    if len == -1 then
        et.G_Printf(""..luaname..".lua: failed to open %s", file)
        return(0)
    end
