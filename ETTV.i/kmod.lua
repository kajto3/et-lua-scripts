...
--		timecounter=timecounter+0.05
	...
	if crazygravity then
		...
		if (crazytime-mtime)/1000 == 0 then
			et.trap_SendConsoleCommand( et.EXEC_APPEND, "qsay ^3Crazygravity: ^7The gravity has been changed to ^:" .. crazygravity_gravity .. "^7!\n" )
			et.trap_SendConsoleCommand( et.EXEC_APPEND, "g_gravity " .. crazygravity_gravity .. "\n" )
			crazydv = 1
		elseif (crazytime-mtime)/1000 == 5 then
			et.trap_SendConsoleCommand( et.EXEC_APPEND, "qsay ^3Crazygravity: ^7The gravity will be changed in ^:5^7 seconds!\n" )
		end
	end
	...
	for i=0, tonumber(et.trap_Cvar_Get("sv_maxclients"))-1, 1 do
		...
		if adrendummy[i] == 0 then
			if tonumber(et.gentity_get(i,"ps.powerups", 12)) > 0 then
				...
				et.trap_SendServerCommand(i, string.format("cp \"^3Adrenaline ^:".. atime .."\n\""))
			else
				adrenaline[i] = 0
				antiloopadr1[i] = 0
				antiloopadr2[i] = 0
...
--[[
	for i=0, tonumber(et.trap_Cvar_Get("sv_maxclients"))-1, 1 do
--		if muted[i] > 0 then
--			et.trap_SendConsoleCommand( et.EXEC_APPEND, "qsay ^3"..mtime.."\n" )
--		end
...
--		end
		if muted[i] ~= 0 then
			if mtime > muted[i] then
				...
				local mute = et.Info_ValueForKey(cs, "mu")
				if mute == 1 then
					local name = et.gentity_get(i,"pers.netname")
					et.trap_SendConsoleCommand( et.EXEC_APPEND, "ref unmute "..i.."\n" )
					et.trap_SendConsoleCommand( et.EXEC_APPEND, "qsay ^3CurseFilter: ^7"..name.." has been unmuted.  Please watch your language!\n" )
				end
				muted[i] = 0
			end
		end
...
--			et.trap_SendConsoleCommand( et.EXEC_APPEND, ""..ks_location.." ^7" ..name.. "^:'s Killing spree was ended! Sprees Disabled!\n" )
			et.trap_SendConsoleCommand( et.EXEC_APPEND, "qsay ^:Killingspree's disabled.  All sprees reset.\n" )
		end
...
function et_InitGame( levelTime, randomSeed, restart )
	...
	et.RegisterModname( "kmod version " .. nrtjversion .. " " .. et.FindSelf() )
	...
	et.G_Print( "nrtj version " .. nrtjversion .. " has been initialized...\n" )
end
...
function spreerecord(PlayerID, kills2)
	local fdadm,len = et.trap_FS_FOpenFile( "spree_record.dat", et.FS_WRITE )
	...
	et.trap_SendConsoleCommand( et.EXEC_APPEND, "qsay ^:New spree record: ^7" ..Name.. " ^7with^3 " ..kills.. "^7 kills  ^7" ..tostring(oldspree).. "\n" )
	loadspreerecord()
end
...
function loadspreerecord()
	local fd,len = et.trap_FS_FOpenFile( "spree_record.dat", et.FS_READ )
	local kills = 0
	local date = ""
	local name = ""
		...
		elseif string.lower(et.trap_Argv(0)) == "ma" or string.lower(et.trap_Argv(0)) == "pma" then
				for i=0,tonumber(et.trap_Cvar_Get("sv_maxclients"))-1,1 do
					if AdminUserLevel(i) >= 2 then
						et.trap_SendServerCommand(i, ("b 8 \"^dPm to admins from ^:SERVER^d --> ^3" .. et.ConcatArgs(1) .. "^7"))
   ...
   if level < 0 or level > 3 then
	if commandSaid then
		...
		commandSaid = false
	    	...
	    	et.G_Print("Admin level does not exist! \[0-3\]\n") 
	end
   return
   end
...
function getsetlvlidfname(name) 
   ...
   if matchcount >= 2 then
	if commandSaid then
		et.trap_SendConsoleCommand( et.EXEC_APPEND, ""..say_parms.." ^3Gib: ^7There are currently ^:" .. matchcount .. "^7 client\[s\] that match \"" ..name.. "\"\n" )
		commandSaid = false
	else
  	    	et.G_Print("There are currently ^:" .. matchcount .. "^7 client\[s\] that match \"" ..name.. "\"\n") 
	end
	return nil
   else
	return slot
...
if confirm == 1 then
	...
	et.trap_FS_FCloseFile( fdadm )
	if commandSaid then
		et.trap_SendConsoleCommand( et.EXEC_APPEND, ""..say_parms.." ^3Setlevel: ^7" .. Name .. "^7 is now a level ^:" .. level .. " ^7user!\n" )
  		commandSaid = false
	else
		et.G_Print( Name .. "^7 is now a level ^:" .. level .. " ^7user!\n" )
	end
	loadAdmins()
	return 1
else
...
function adminStatus(PlayerID)
	...
	if finger then
		...
		if ( AdminLV3[GUID] ) then
			et.trap_SendConsoleCommand( et.EXEC_APPEND, ""..say_parms.."^7" .. name .. " ^:status (^3#Manager^:)\n" )
		elseif ( AdminLV2[GUID] ) then
			et.trap_SendConsoleCommand( et.EXEC_APPEND, ""..say_parms.."^7" .. name .. " ^:status (^3#Advanced^:)\n" )
		elseif ( AdminLV1[GUID] ) then
			et.trap_SendConsoleCommand( et.EXEC_APPEND, ""..say_parms.."^7" .. name .. " ^:status (^3#Trusted^:)\n" )
		else
			et.trap_SendConsoleCommand( et.EXEC_APPEND, ""..say_parms.."^7" .. name .. " ^:status (^3#Unknown^:)\n" )
		end
		finger = false
		commandSaid = false
	else
		if ( AdminLV3[GUID] ) then
			et.trap_SendConsoleCommand( et.EXEC_APPEND, ""..say_parms.." ^:Client status (^3#Manager^:)\n" )
		elseif ( AdminLV2[GUID] ) then
			et.trap_SendConsoleCommand( et.EXEC_APPEND, ""..say_parms.." ^:Client status (^3#Advanced^:)\n" )
		elseif ( AdminLV1[GUID] ) then
			et.trap_SendConsoleCommand( et.EXEC_APPEND, ""..say_parms.." ^:Client status (^3#Trusted^:)\n" )
		else
			et.trap_SendConsoleCommand( et.EXEC_APPEND, ""..say_parms.." ^:Client status (^3#Unknown^:)\n" )
		end
	end
end
...
function burn(client) 
   ...
   if clientnum then 
      if (clientnum >= 0) and (clientnum < 64) then 
		...
		if commandSaid then
			et.trap_SendConsoleCommand( et.EXEC_APPEND, ""..say_parms.." ^3Slap: ^7Client is not activly playing\n" )
			commandSaid = false
		else
	            et.G_Print("Client is not activly playing\n") 
		end
         return 
         end 
   ...
   if clientnum then 
      if (clientnum >= 0) and (clientnum < 64) then 
		...
		if commandSaid then
         ...
         if et.gentity_get(clientnum,"sess.sessionTeam") >= 3 or et.gentity_get(clientnum,"sess.sessionTeam") < 1 then
		if commandSaid then
			et.trap_SendConsoleCommand( et.EXEC_APPEND, ""..say_parms.." ^3Gib: ^7Client is not activly playing\n" )
			commandSaid = false
		else
	            et.G_Print("Client is not activly playing\n") 
		end
...
function spec999()
	...
	et.trap_SendConsoleCommand( et.EXEC_APPEND, ""..say_parms.." ^3Spec999: ^7Moving ^:" ..matches.. " ^7players to spectator\n" )
end
...
function ModInfo(PlayerID)
	et.trap_SendServerCommand( PlayerID,"cpm \"^:[^3#//Data.hourb.com ^:]\n\"")
end

function advPlayers(PlayerID)
	et.trap_SendServerCommand(PlayerID, string.format("print \"^3 ID ^:: ^3Player                     Rate  Snaps\n"))
	...
	for i=0, tonumber(et.trap_Cvar_Get("sv_maxclients"))-1, 1 do
		...
		if et.gentity_get(i,"pers.connected") ~= 2 then
			...
			et.trap_SendServerCommand(PlayerID, string.format('print "%s^7%2s ^::^7 %s%s %5s  %5s %s\n"',team[teamnumber], i, name, space, rate, snaps, ref))
...
function admins(PlayerID)
	et.trap_SendServerCommand(PlayerID, string.format("print \"^3 ID ^::^3 Player                   ^:: ^3 Level ^: : ^3 AdminName\n"))
	et.trap_SendServerCommand(PlayerID, string.format("print \"^:----------------------------------------------------------------\n"))
	local team = { "^:X" , "^4L" , " " }
	local playercount = 0
	local spa = 23
	...
	for i=0, tonumber(et.trap_Cvar_Get("sv_maxclients"))-1, 1 do
		...
		if AdminUserLevel(i) == nil or AdminUserLevel(i) == " " or AdminUserLevel(i) < 1 then
			...
			et.trap_SendServerCommand(PlayerID, string.format('print "%s^7%2s ^::^7 %s%s ^::  %5s  ^::^7  ^7%s\n"',team[teamnumber], i, name, space, level, adname))
			playercount = playercount + 1
...
function curse_filter( PlayerID )
	...
	if k_cursemode == 1 then
		muted[PlayerID] = mtime + 30000         --5 minutes in milliseconds
		et.trap_SendConsoleCommand( et.EXEC_APPEND, "ref mute "..PlayerID.."\n" )
		et.trap_SendConsoleCommand( et.EXEC_APPEND, "qsay ^3CurseFilter: ^7"..name.." ^7has been auto muted for ^:5^7 minutes!\n" )
--		et.trap_SendConsoleCommand( et.EXEC_APPEND, "qsay ^3"..muted[PlayerID].."\n" )
	end
end
--]]
...
function log_chat( PlayerID, mode, text, PMID )
	...
	elseif mode == et.SAY_TEAM then
		local name = et.gentity_get(PlayerID,"pers.netname")
		local name = et.Q_CleanStr( name )
		LOG = "("..time..") "..name.." (TEAM): " ..text.. "\n"
	elseif mode == et.SAY_BUDDY then
		local name = et.gentity_get(PlayerID,"pers.netname")
		local name = et.Q_CleanStr( name )
		LOG = "("..time..") "..name.. " (BUDDY): " ..text.. "\n"
	...
	elseif mode == "VSAY_TEAM" then
		local name = et.gentity_get(PlayerID,"pers.netname")
		local name = et.Q_CleanStr( name )
		LOG = "("..time..") "..name.. " (VSAY_TEAM): " ..text.. "\n"
	elseif mode == "VSAY_BUDDY" then
		local name = et.gentity_get(PlayerID,"pers.netname")
		local name = et.Q_CleanStr( name )
		LOG = "("..time..") "..name.. " (VSAY_BUDDY): " ..text.. "\n"
	elseif mode == "VSAY_ALL" then
		local name = et.gentity_get(PlayerID,"pers.netname")
		local name = et.Q_CleanStr( name )
		LOG = "("..time..") "..name.. " (VSAY): " ..text.. "\n"
	elseif mode == "PMESSAGE" then
		...
		if rec1 then
			...
			LOG = "("..time..") "..from.." to "..recipiant.." (PRIVATE): " ..text.. "\n"
		end
				...
				if et.gentity_get(clientNum,"health") > 0 then
					if selfkills[clientNum] < k_slashkills then
						...
						elseif selfkills[clientNum] == (k_slashkills - 1) then
							if k_advancedpms == 1 then
								et.trap_SendConsoleCommand( et.EXEC_APPEND, "m2 " .. name .. " ^7You have ^:1^7 /kill left for this map.\n" )
								et.G_ClientSound(clientNum, pmsound)
							else
								et.trap_SendConsoleCommand( et.EXEC_APPEND, "m " .. name .. " ^7You have ^:1^7 /kill left for this map.\n" )
							end
						end
					else
						if k_advancedpms == 1 then
		...
		for level,comm,str in string.gfind(filestr, "[^%#](%d)%s*%-%s*([%w%_]*)%s*%=%s*([^%\n]*)") do
			...
			if (string.lower(BangCommand) == comm2 ) then
				if tonumber(level) <= AdminUserLevel(PlayerID) then
					...
					et.trap_SendConsoleCommand( et.EXEC_APPEND, "qsay ^3"..comm..":^7 Insufficient Admin status\n" )
				end
			end
		end
	end
...
--Admin commands
  ...
  elseif (string.lower(BangCommand) == k_commandprefix.."readconfig" ) then
	if AdminUserLevel(PlayerID) == 3 then
		et.trap_SendConsoleCommand( et.EXEC_APPEND, "exec nrtj.cfg ; qsay ^3ReadConfig:^7 Config reloaded\n" )
		readconfig()
	else
  ...
  elseif (string.lower(BangCommand) == "!makeshoutcaster" ) then
	if AdminUserLevel(PlayerID) == 3 then
		if Cvarct < 3 then
  ...
  elseif (string.lower(BangCommand) == "!removeshoutcaster" ) then
	if AdminUserLevel(PlayerID) == 3 then
		if Cvarct < 3 then
  ...
  elseif (string.lower(BangCommand) == "!makereferee" ) then
	if AdminUserLevel(PlayerID) == 3 then
		if Cvarct < 3 then
  ...
  elseif (string.lower(BangCommand) == "!removereferee" ) then
	if AdminUserLevel(PlayerID) == 3 then
		if Cvarct < 3 then
  ...
  elseif (string.lower(BangCommand) == "!gravity" ) then
	if AdminUserLevel(PlayerID) == 3 then
		if Cvarct < 3 then
  ...
  elseif (string.lower(BangCommand) == "!knifeonly" ) then
	if AdminUserLevel(PlayerID) == 3 then
		if Cvarct < 3 then
  ...
  elseif (string.lower(BangCommand) == "!speed" ) then
	if AdminUserLevel(PlayerID) == 3 then
		if Cvarct < 3 then
  ...
  elseif (string.lower(BangCommand) == "!knockback" ) then
	if AdminUserLevel(PlayerID) == 3 then
		if Cvarct < 3 then
  ...
  elseif (string.lower(BangCommand) == "!cheats" ) then
	if AdminUserLevel(PlayerID) == 3 then
		if Cvarct < 3 then
  ...
  elseif (string.lower(BangCommand) == "!laser" ) then
	if AdminUserLevel(PlayerID) == 3 then
		if Cvarct < 3 then
  ...
  elseif (string.lower(BangCommand) == k_commandprefix.."sniperwar" ) then
	...
	--level 2
  if (string.lower(BangCommand) == "!kick" ) then
  ...
  elseif (string.lower(BangCommand) == "!warn" ) then
	if AdminUserLevel(PlayerID) >= 2 then
		if Cvarct < 3 then
  ...
  elseif (string.lower(BangCommand) == "!mute" ) then
	if AdminUserLevel(PlayerID) >= 2 then
		if Cvarct < 3 then
  ...
  elseif (string.lower(BangCommand) == "!timelimit" ) then
	if AdminUserLevel(PlayerID) >= 2 then
  ...
  elseif (string.lower(BangCommand) == "!unmute" ) then
	if AdminUserLevel(PlayerID) >= 2 then
		if Cvarct < 3 then
...
function lstcomds( PlayerID )
	if AdminUserLevel(PlayerID) >= 0 then
		...
		for q=1, tonumber(lvl0c), 3 do
			...
			et.trap_SendServerCommand(PlayerID, string.format('print \"%21s^:|^7 %21s^:|^7 %21s^:|^7\n"', lvl0[b], lvl0[m], lvl0[e]))
		end
		et.trap_SendServerCommand(PlayerID, string.format("print \"^:-------------------------------------------------------------------\n\n"))
	end
	...
	if AdminUserLevel(PlayerID) == 3 then
		et.trap_SendServerCommand(PlayerID, string.format("print \"Level 3 Commands \n"))
		et.trap_SendServerCommand(PlayerID, string.format("print \"^:-------------------------------------------------------------------\n"))
		...
		if victimteam == killerteam and killer ~= victim and killer ~= 1022 then
			if AdminUserLevel(killer) < k_tk_protect then
				...
				if warning > teamkillr[killer] and k_tklimit_low < teamkillr[killer] then
					if k_advancedpms == 1 then
						et.trap_SendConsoleCommand( et.EXEC_APPEND, "m2 " .. killername .. " ^:You are making to many teamkills please be more careful or you will be kicked!\n" )
						et.G_ClientSound(killer, pmsound)
					else
						et.trap_SendConsoleCommand( et.EXEC_APPEND, "m " .. killername .. " ^:You are making to many teamkills please be more careful or you will be kicked!\n" )
					end
...
--			et.trap_SendServerCommand( killer,"cp \"^:KILLINGSPREE!\"\n")
			et.trap_SendConsoleCommand( et.EXEC_APPEND, ""..ks_location.." "..str.."\n" )
...
--			et.trap_SendServerCommand( killer,"cp \"^:RAMPAGE!\"\n")
			et.trap_SendConsoleCommand( et.EXEC_APPEND, ""..ks_location.." "..str.."\n" )
			if k_spreesounds == 1 then
				if k_noisereduction == 1 then
...
--			et.trap_SendServerCommand( killer,"cp \"^:DOMINATION!\"\n")
			et.trap_SendConsoleCommand( et.EXEC_APPEND, ""..ks_location.." "..str.."\n" )
			if k_spreesounds == 1 then
				if k_noisereduction == 1 then
...
--			et.trap_SendServerCommand( killer,"cp \"^:UNSTOPABLE!\"\n")
			et.trap_SendConsoleCommand( et.EXEC_APPEND, ""..ks_location.." "..str.."\n" )
			if k_spreesounds == 1 then
				if k_noisereduction == 1 then
...
--			et.trap_SendServerCommand( killer,"cp \"^:GODLIKE!\"\n")
			et.trap_SendConsoleCommand( et.EXEC_APPEND, ""..ks_location.." "..str.."\n" )
			if k_spreesounds == 1 then
				if k_noisereduction == 1 then
...
--			et.trap_SendServerCommand( killer,"cp \"^:WICKED SICK!\"\n")
			et.trap_SendConsoleCommand( et.EXEC_APPEND, ""..ks_location.." "..str.."\n" )
			if k_spreesounds == 1 then
				if k_noisereduction == 1 then
...
--				et.trap_SendServerCommand( killer,"cp \"^:FLAKMONKEY!\"\n")
